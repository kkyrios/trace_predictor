...
long_model = lstm_reg(feature, 4).to(device)  # 实例化
short_model = lstm_reg(feature, 4).to(device)
# 定义损失函数、优化器
criterion = nn.MSELoss()  # 均方误差
optimizer = torch.optim.Adam(long_model.parameters(), lr=1e-2)  # 优化器
short_train_range_l = long_train_range_l = lower_bound
short_train_range_r = long_train_range_r = train_size
...
# 前70%使用原模型
train(700)
long_model = long_model.eval().to(device)  # 转换成测试模式
...
pred_test = long_model(var_data)  # 整个数据集的拟合结果
pred_test = pred_test.cpu().view(-1).data.numpy()
print("len", len(data_X))

# 每预测50个样本更新长周期模型（从头开始，权重50%），每预测2个样本更新短周期模型（最近100个样本，权重50%）
for i in range(train_size, len(data_X) + lower_bound):
    ...
    # 预测
    single_prediction_output = long_model(var_single_prediction_input) * 0.3 + short_model(var_single_prediction_input) * 0.7
    ...
    # 更新长周期模型
    if (i - train_size) % 50 == 0:
        print(i - train_size, '/', upper_bound - train_size, ' long_model updated')
        long_train_range_l += 50
        long_train_range_r += 50
    ...
    # 更新短周期模型
    if (i - train_size) % 5 == 0:
        print(i - train_size, '/', upper_bound - train_size, ' short_model updated')
        long_train_range_l += 5
        long_train_range_r += 5
        train_X = data_X[i - lower_bound - 100:i - lower_bound]
        train_Y = data_Y[i - lower_bound - 100:i - lower_bound]
        train_X = train_X.reshape(-1, 1, feature)
        train_Y = train_Y.reshape(-1, 1, 1)
        train_x = torch.from_numpy(train_X).to(device)
        train_y = torch.from_numpy(train_Y).to(device)
...
for i in range(train_size, len(data_X) + lower_bound):
    ...
    var_single_prediction_input = Variable(single_prediction_input).to(device)
    # 预测
    single_prediction_output = long_model(var_single_prediction_input)
    single_prediction_output = single_prediction_output[0, 0, 0].cpu().detach().numpy()
    # 添加元素
    pred_test = np.append(pred_test, single_prediction_output)
